...
public:	
	 ...
	 void push(int val){
         ...
         while(v[childidx]<v[parentind]){ //childnode value is smaller then the parent node value then swap the value of childnode with the parent node until all node will not satify the property of min heap.
         	swap(v[childidx],v[parentind]);
         	childidx=parentind;
         	parentind=(childidx-1)/2;
         	if(parentind<0){ //or childidx==0 //childnode is the root node and it saatify the min heap property so no need to check..
         		break;
         	}
         }
